...
import ResultOverlay from '@/components/ResultOverlay'; // Re-integrated the component usage
...
export default function PlayPage() {
  ...
  // FIX: Optimized Preloading (Only next 2, async)
  useEffect(() => {
    if (deck.length > 1) {
      const toPreload = deck.slice(1, 3);
      for (const img of toPreload) {
        const i = new Image();
        i.decoding = 'async';
        i.loading = 'eager'; // optional for JS Image object but good practice
        i.src = img.url;
      }
    }
  }, [deck]);
  ...
  async function fetchMoreCards(isInitial = false, forceCategory?: ImageCategory) {
    ...
    try {
      ...
      setDeck(prev => {
        ...
        const hist = historyRef.current; // Use Ref to avoid closure staleness

        let newUnique = images.filter(img => !currentIds.has(img.id) && !hist.includes(img.id));

        // Emergency refill if exhausted
        if (newUnique.length < 3 && images.length > 0) {
             setHistory([]); // Reset history if we run out of unique cards
        ...
        <div className="relative flex flex-col justify-center">
          <div className="relative aspect-[4/3] md:aspect-auto md:h-[600px] w-full">
            <GameCard
                ...
                onSkip={nextCard}
                isActive={true} // FIX: Tell Card this is the active one
            />
