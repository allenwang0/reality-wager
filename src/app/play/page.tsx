      ...
      <div className="max-w-md w-full border-2 border-protocol-signal p-6 bg-black relative shadow-[0_0_50px_rgba(0,255,50,0.1)]">
        ...
        <div className="space-y-6 text-sm font-mono text-gray-300 mb-8">
          <div>
            ...
            <p>Real (Photo) vs AI (Generated).</p>
          </div>
          <div>
            <strong className="text-white block mb-1">2. WAGER</strong>
            <p>High Wager = High Multiplier.</p>
          </div>
          <div>
            <strong className="text-white block mb-1">3. CONSEQUENCE</strong>
            <p>Debt leads to manual labor.</p>
          </div>
        </div>
        <button
          onClick={onClose}
  ...
  if (isBankrupt) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-black text-white p-6 text-center font-mono border-4 border-red-900">
        ...
        <Link
          href="/back-room"
          className="protocol-btn px-8 py-4 text-white border-white hover:bg-white hover:text-black transition-all"
        >
          [ ENTER BACK ROOM ]
        </Link>
      </div>
      ...
      <div className="flex justify-between items-end border-b border-protocol-gray pb-4 mb-4 z-20 relative bg-black/50 backdrop-blur-sm">
        <div className="flex items-end gap-4">
          ...
          <button
            ...
            className="mb-1 text-[10px] text-protocol-signal border border-protocol-signal px-2 py-1 hover:bg-protocol-signal hover:text-black"
          >
            [?] HELP
          </button>
        </div>
      ...
      <div className="grow grid grid-cols-1 md:grid-cols-[1fr_350px] gap-6">
            ...
            {deck
              ...
              .map((image, index, array) => {
                ...
                return (
                  ...
                  >
                    <GameCard src={image.url} onSkip={nextCard} isActive={isTopCard} />
        ...
        {!result && (
          <div className="flex flex-col space-y-4 z-20">
                ...
                <div className="grid grid-cols-4 gap-1">
                  {['general', 'faces', 'places', 'art'].map((cat) => (
                    <button
                      ...
                      className={`text-[9px] uppercase py-2 border ${
                        category === cat ? 'bg-white text-black' : 'text-gray-500 border-gray-800'
                      }`}
                    >
                      {cat}
                    </button>
              ...
              <div className="bg-protocol-dark border border-protocol-gray p-3">
                <div className="flex justify-between items-center mb-3">
                  ...
                  <div className="flex space-x-2">
                    <button
                      ...
                      className={`text-[10px] ${wagerMode === 'percent' ? 'text-white' : 'text-gray-600'}`}
                    >
                      %
                    </button>
                    <button
                      onClick={() => setWagerMode('custom')}
                      className={`text-[10px] ${wagerMode === 'custom' ? 'text-white' : 'text-gray-600'}`}
                    >
                      $
                    </button>
                  </div>
                ...
                {wagerMode === 'percent' ? (
                    ...
                    {[10, 25, 50, 100].map((pct) => (
                      <button
                        ...
                        className={`py-2 text-[10px] font-bold border ${
                          wagerPercent === pct ? 'bg-white text-black' : 'border-gray-700 text-gray-500'
                        }`}
                      >
                        {pct}%
                      </button>
                  ...
                  <input
                    ...
                    placeholder="Amount"
                    className="w-full bg-black border border-gray-600 p-2 text-white font-mono mb-3"
                  />
                )}
            ...
            {errorMsg && (
              <div className="text-center text-[10px] text-protocol-noise border border-protocol-noise p-2">{errorMsg}</div>
            )}
            ...
            <div className="grid grid-cols-2 gap-4 mt-auto">
              <button
                ...
                className="h-20 md:h-24 bg-transparent text-white font-bold text-xl border border-gray-600 hover:bg-white hover:text-black transition-all"
              >
                REAL
              </button>
              <button
                ...
                disabled={loadingResult}
                className="h-20 md:h-24 bg-transparent text-white font-bold text-xl border border-gray-600 hover:bg-protocol-noise hover:text-white transition-all"
              >
                AI
              </button>
            </div>
